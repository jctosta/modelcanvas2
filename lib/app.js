...
import { markdown } from './markdown-template';
...
export default class App {
	...
	static createMarkdown(instance, parent) {
		const template = Handlebars.compile(markdown);
		const canvas = {
			name: parent.properties.name,
			description: parent.properties.description,
			children: App.listChildren(instance, parent)
				.map(c => { 
					return { 
						title: c.properties.title, 
						description: c.properties.description,
						children: App.listChildren(instance, c).map(c => c.properties.content),
					}; 
				}),
		};
		return template({ canvas });
	}
	static generateTileMarkdown(instance, parent) {
		return `
.left-column[
...
${App.listChildren(instance, parent).map(c => '- ' + c.properties.content).join('\n')}
]
		`;
	}
	static generateMarkdown(instance, parent) {
		const markdown = `
...
${App.listChildren(instance, parent).map(c => App.generateTileMarkdown(instance, c)).join('\n\n---')}
		`;
		return markdown;
	}	
}